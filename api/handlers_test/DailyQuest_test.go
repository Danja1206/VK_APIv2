...
func TestDailyQuest(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "/daily-reward", bytes.NewBuffer(jsonData))
	...
	handler := http.HandlerFunc(internal.WithDBContextMiddleware(handlers.DailyQuest, db))
